...
export function bindExecutionOverlayContext(frame, value) {
    ...
    exactKeys(
        ...
        [
            ...
            'off_box_transmissions_observed',
            'evidence_href',
        ],
    ...
    if (
        context.off_box_transmissions_observed !== undefined &&
        (!Number.isSafeInteger(context.off_box_transmissions_observed) ||
            context.off_box_transmissions_observed < 0 ||
            context.external_network_calls !== 'observed')
    ) {
        throw new Error('execution overlay presentation context has an invalid off-box observation count')
    }
    if (
...
export function executionOverlayPresentation(
    frame,
    context,
    mediaElapsedMs,
    ...
    const secondaryLabels = [
        ...
        bound?.external_network_calls === 'none'
            ...
            : bound?.external_network_calls === 'observed'
              ? bound?.off_box_transmissions_observed === 0 ||
                (networkObservation?.browser_network === 'observed' &&
                    networkObservation?.off_box_transmissions_observed === 0)
                  ? 'Local app traffic only · no off-box transmission'
                  : 'Application network traffic observed'
