...
export function scoreWorkflowQualification(form) {
    ...
    const hasBoundedIntent = form.inputStructure !== 'open_ended'
    const isStableEnough = form.stability !== 'changes_weekly'
    const passesFitGates =
        needsUiActuation && hasVerifier && hasBoundedIntent && isStableEnough

    if (score >= 20 && hasBudget && passesFitGates) {
        return { score, tier: 'priority' }
    }
    if (score >= 12 && passesFitGates) {
        return { score, tier: 'review' }
    }
    return { score, tier: 'community' }
}

export const QUALIFICATION_SALES_TASK_SCHEMA =
    'openadapt.qualification-sales-task/v1'

export function buildQualificationSalesTask({
    id,
    qualification,
    sourceRoute,
    submittedAt,
}) {
    if (!id || !qualification?.tier || !Number.isInteger(qualification?.score)) {
        throw new Error('A sales task requires an id and a scored qualification')
    }
    ...
    return Object.freeze({
        ...
        salesTaskId: id,
        salesTaskStatus: 'new',
        sourceRoute: sourceRoute || '/qualify',
        bookingState: 'not_booked',
        submittedAt,
        qualificationScore: String(qualification.score),
